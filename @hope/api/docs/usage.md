...
`@hope/api` 是所有服务包共享的 HTTP 客户端容器，采用 **IoC（控制反转）** 架构设计：
...
```typescript
import { useDelete } from '@hope/api'
...
```typescript
import { useUpload } from '@hope/api'
...
```typescript
import { pathToUrl } from '@hope/api'
...
import { useGet, usePost, usePut, useDelete, pathToUrl } from '@hope/api'
...
import { useGet, usePost } from '@hope/api'
...
import type { ApiClient } from '@hope/api'
...
`@hope/api` 处理同步请求，`@hope/realtime` 处理流式请求：
